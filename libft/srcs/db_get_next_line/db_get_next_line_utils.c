...
t_bufferList	*ftlst_new_buffer(void)
{
	...
	new = db_malloc(sizeof(t_bufferList));
	if (!new)
		return (NULL);
	new->content = db_malloc(BUFFER_SIZE + 1);
	if (!(new->content))
		return (db_free(new), NULL);
	(new->content)[0] = '\0';
	new->next = 0;
	return (new);
}
